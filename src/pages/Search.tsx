...
const Search = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [profilesRes, servicesRes, categoriesRes, reviewsRes, requestsRes] = await Promise.all([
      ...
      (supabase as any).from("public_service_requests").select("id, requester_type, description, budget, city, state, latitude, longitude, category_id, profile_id, service_categories(name)").neq("origin", "radar").order("created_at", { ascending: false }),
    ]);
