...
ALTER TABLE public.service_requests
  ...
  ADD COLUMN IF NOT EXISTS search_radius numeric NOT NULL DEFAULT 3;
...
AS $$
  ...
  SELECT
    ...
    -- privacy: coarse coordinates (~100m grid) before matching
...
REVOKE ALL ON FUNCTION public.find_nearby_providers(double precision, double precision, numeric, uuid, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.find_nearby_providers(double precision, double precision, numeric, uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.find_nearby_providers(double precision, double precision, numeric, uuid, integer) TO service_role;
