...
AS $function$
  WITH live AS (
    ...
    WHERE auth.uid() IS NOT NULL
      ...
      -- bots de demonstração não atualizam posição continuamente
      AND (COALESCE(p.is_synthetic, false) OR pl.updated_at > now() - interval '15 minutes')
      AND (_include_synthetic OR COALESCE(p.is_synthetic, false) = false)
      AND (
        COALESCE(p.is_synthetic, false) = false
        OR p.synthetic_expires_at IS NULL
        OR p.synthetic_expires_at > now()
      )
      AND (
